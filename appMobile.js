...
let camera;
// Verifica se o dispositivo está em modo retrato ou paisagem
let isLandscape = window.innerWidth > window.innerHeight;
// Configurações da câmera para dispositivos móveis na horizontal
function setCamera(){
    if (isLandscape) {
        ...
        camera = new THREE.OrthographicCamera(-orthoSize * aspect / 2 - 1, orthoSize * aspect / 2 + 1, // left, right
...
const materials = [csgFinalMaterial, csgFinalMaterial, csgFinalMaterial, csgFinalMaterial];
...
playerLoader.load('./assets/scene.gltf', function(gltf){
    ...
    gltfInitialPosition = new THREE.Vector3(-0.3, size.positionY/2, planeZ/2-2)
...
const raycasterGeometry = new THREE.BoxGeometry(playerLength, 1, csgFinal.geometry.boundingSphere.radius + 0.5);  
const draggableZ = planeZ/2 - raycasterGeometry.parameters.depth/2;
let draggableRaycaster = new THREE.Mesh(raycasterGeometry, new THREE.MeshLambertMaterial({ color: 0xffffff, transparent: true, opacity: 0.5}));
    draggableRaycaster.position.set(0, size.positionY, draggableZ);
const draggableInitialPosition = new THREE.Vector3().copy(draggableRaycaster.position);
scene.add(draggableRaycaster);
 
const dragControl = new DragControls([draggableRaycaster], camera, renderer.domElement);
...
dragControl.addEventListener ('drag', function(event){
    event.object.position.z = draggableZ;
    event.object.position.y = size.positionY;
    if (event.object.position.x - playerLength/2 < LEFT_LIMIT){
        event.object.position.x = LEFT_OFFSET;
        csgFinal.position.x = LEFT_OFFSET;
        gltfPlayer.position.x = LEFT_OFFSET - 0.3;
    } else if (event.object.position.x + playerLength/2 > -LEFT_LIMIT){ //rightWall
        event.object.position.x = RIGHT_OFFSET;
        csgFinal.position.x = RIGHT_OFFSET;
        gltfPlayer.position.x = RIGHT_OFFSET - 0.3;
    } else {
        csgFinal.position.x = event.object.position.x;
        gltfPlayer.position.x = event.object.position.x - 0.3;
    }
    csgFinal.bb.center.copy(csgFinal.position)
...
function resetPosition(){
    ...
    draggableRaycaster.position.copy(draggableInitialPosition);
